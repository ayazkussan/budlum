...
**2026-09-12 08:41 UTC, head `052216b` (pushed).** Ask-user round (user requested questions; answers: consumer=NOW, fmt=REFORMAT manually, main-junk=leave+documented, dep-review=ignore, lubot=continue, PR-policy=keep). Executed: `c1c42eb` nightly E0277 (&String<&str) + TWO verifier holes closed at read-back: both-sided exclusion lacked straddle ordering (revoked q could ride an unrelated honest adjacent pair -> false "clear"), one-sided arms lacked index==0 / index==count-1 position pins; +3 assertions (fake-gap Malformed, shifted-upper refused, honest above-max walks). `052216b` CONSUMER: verify_identity_claim (witness -> RootBinding -> SubjectBinding; calendar deliberately left to is_credential_valid) + RPC bud_identityVerifyAtAnchor (api trait + server handler: shape errors -32602, VERIFIED refusals answered accepted:false+reason; CredentialCommitment has Deserialize - checked) + mod.rs re-exports + end-to-end claim test (revoked=Witness(Revoked); stale witness vs new anchor=RootMismatch). Toolchain hosts (static.rust-lang.org, release-assets) UNREACHABLE (measured again) - no local rustfmt: fmt-131 queued as NEXT-TURN first fetch of Budlum Core --log-failed retry loop; do NOT blind-guess 131 hunks. New-head verdicts to read at turn-open: expect indexing=0, brands=11 stays red BY USER CHOICE (main junk via merge-ref; same file drives Typos - documented, not branch-fixable), doctest suites should be green; ASan/Miri must be green after c1c42eb compile fix. Then lubot: compact decision + re-clone 31-patch series. Mode: push-first, thousands-of-lines, no reports.
